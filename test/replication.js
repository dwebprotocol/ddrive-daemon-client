...
test('can cancel an active download', async t => {
  ...
  try {
    ...
    t.true(fileStats.get('/a/2').downloadedBlocks < 1000 && fileStats.get('/a/2').downloadedBlocks > 0)
  } catch (err) {
    t.fail(err)
  }
  ...
  async function writeFile (drive, name, numBlocks) {
    ...
    return new Promise((resolve, reject) => {
      ...
      for (let i = 0; i < numBlocks; i++) {
        writeStream.write(Buffer.alloc(1024).fill('abcdefg'))
      }
      writeStream.end()
    })
  }
...
// TODO: The latency option is not supported in the new daemon, so this is not used.
...
// TODO: The Peers API needs to be revamped.
test.skip('can get peer info globally', async t => {
  const { clients, cleanup } = await create(3)
  const firstClient = clients[0]
  const secondClient = clients[1]
  const thirdClient = clients[2]
...
// TODO: The Peers API needs to be revamped.
test.skip('can get peer info for one discovery key', async t => {
  const { clients, daemons, cleanup } = await create(3)
  const firstClient = clients[0]
  const secondClient = clients[1]
  const thirdClient = clients[2]
