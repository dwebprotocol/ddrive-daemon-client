...
class DaemonCommand extends Command {
  ...
  static drivePathArg (info) {
    return {
      name: 'path',
      description: 'The path to the drive (must be within ~/DDrive)',
      parse: path => {
        if (!path) return constants.mountpoint
        path = p.resolve(path)
        if (path.startsWith(constants.networkPath)) throw new Error('dDrive commands cannot be run on the Network directory.')
        if (path.startsWith(constants.mountpoint)) return path
        throw new Error(`Path ${path} is not contained within the dDrive mountpoint`)
      },
      ...info
    }
  }
  ...
  async run () {
    try {
      this.client = new DDriveClient()
      await this.client.ready()
    } catch (err) {
      console.error('Could not connect to the daemon. Is it running? Check with `ddrive status`.')
      this.exit(1)
    }
  }
}
