...
module.exports = class DriveClient {
  ...
  _addCompatMethods (drive) {
    ...
    drive.stats = async (opts, cb) => {
      ...
      for (const [path, { metadata, content }] of mounts) {
        stats.push({
          path,
          metadata: await getBaseStats(metadata),
          content: await getBaseStats(content)
        })
      }

      return { stats, network }

      async function getBaseStats (base) {
        if (!base) return {}
        const stats = base.stats
        const networkingStats = {
          key: base.key,
          discoveryKey: base.discoveryKey,
          peers: base.peers.length
        }
        return {
          ...networkingStats,
          ...
          downloadedBlocks: opts.networkingOnly ? 0 : await base.downloaded(),
          totalBlocks: base.length
        }
      }
    }
  ...
  // ddrive-daemon-client works both with Promises and callbacks. ddrive.promises is exclusively promises.
  ...
  async _createDrive (opts) {
    const store = this._client.basestore()
    var drive = ddrive(store, opts && opts.key, {
      ...opts,
      extension: false
    }).promises
    await drive.ready()
